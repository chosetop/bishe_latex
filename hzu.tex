...
AI 虚拟试穿模块采用异步任务架构，服务端由”任务创建接口 + Worker 消费进程”构成。前端 Nuxt3 负责提交试穿参数并轮询任务状态，完成结果展示。本模块参考了 DrapeNet\cite{drapenet_garment_generation}的服装生成与自监督拟合思路，结合扩散模型进行真人效果合成，生成图像质量采用结构相似性（SSIM）\cite{wang2004ssim}等指标衡量。
...
注册与登录功能测试过程中，系统能够对账号不存在、密码错误等异常输入给出明确的错误提示，并在认证通过后正确跳转并展示登录态下的页面内容；其中，图~\ref{fig:test-login-fail}展示了登录失败时的提示信息，图~\ref{fig:test-login-success}展示了登录成功后的页面与会话状态。
